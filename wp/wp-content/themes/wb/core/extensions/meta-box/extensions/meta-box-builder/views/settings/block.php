...
?>
<h2 ng-show="meta.for == 'block'"><?php esc_html_e( 'Options', 'meta-box-builder' ); ?></h2>

<table class="form-table mbb-settings-block" ng-show="meta.for === 'block'">
	<tr>
...
<h2 ng-show="meta.for == 'block'"><?php esc_html_e( 'Block Supports', 'meta-box-builder' ); ?></h2>
...
<table class="form-table mbb-settings-block" ng-show="meta.for === 'block'">
	...
	<tr ng-show="meta.render_with === 'code'">
		...
		<td>
			...
			<p class="description"><?php printf( __( 'Supports <a href="%s" target="_blank">Twig template engine</a> with additional variables, functions:', 'meta-box-builder' ), 'https://twig.symfony.com/doc/1.x/templates.html' ); ?></p>
			<ul>
				<li><code ng-non-bindable>{{ attribute }}</code> - <?= __( 'Block attribute. Replace <code>attribute</code> with <code>anchor</code>, <code>align</code> or <code>className</code>).', 'meta-box-builder' ); ?></li>
				<li><code ng-non-bindable>{{ field_id }}</code> - <?= __( 'Field value. Replace <code>field_id</code> with a real field ID.', 'meta-box-builder' ); ?></li>
				<li><code>mb.function()</code> - <?= __( 'Run PHP/WordPress function via <code>mb</code> namespace. Replace <code>function</code> with a valid PHP/WordPress function name.', 'meta-box-builder' ); ?></li>
			</ul>
		</td>
	</tr>
	<tr>
...
<p class="description" ng-show="meta.for === 'block'"><?php esc_html_e( 'Supported variables (no trailing slash):', 'meta-box-builder' ) ?></p>
<table ng-show="meta.for === 'block'">
	<tr>
